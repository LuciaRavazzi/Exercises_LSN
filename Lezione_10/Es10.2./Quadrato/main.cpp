...
int main(int argc, char* argv[]){
  ...
  if(rank == 0){
    for(int i=0; i<N_city; i++)ct[i].Set(&rnd);
  }
 ...
 for (i = 0; i < N_city - 1; i++) {
  ...
  for (j = i + 1; j < N_city; j++){
   ...
   double b=sqrt(ct[appo].GetX()*ct[appo].GetX()+ct[appo].GetY()*ct[appo].GetY());
   if(a < b) appo = j;
  }
  tempx=ct[i].GetX();
  tempy=ct[i].GetY();
  ct[i].SetX(ct[appo].GetX());
  ct[i].SetY(ct[appo].GetY());
  ct[appo].SetX(tempx);
  ct[appo].SetY(tempy);
 }
 ...
 //Devo dire a tutti gli altri chi è il più piccolo.
