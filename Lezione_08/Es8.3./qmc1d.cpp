...
opportunely initialized by the initialize() function. */
	for(int i=0;i<equilibration;i++)
	{
		...
		{
			brownianMotion(LEFT,&rnd);
			brownianMotion(RIGHT,&rnd);
		}
		
		translation(&rnd); //Mi muovo (forse) con il Metropolis.
...
double potential_density_matrix(double val, double val_next)
{
	...
	return dens_left+dens_right; //è l'esponente infatti fa la somma.
}
...
{
	//double a=exp(-pow(x-mu,2.)/(2.*pow(sigma,2.)))*(-1./pow(sigma,2.)+pow(x-mu,2.)/(2*pow(sigma,2.)));
	//double b=exp(-pow(x+mu,2.)/(2.*pow(sigma,2.)))*(-1./pow(sigma,2.)+pow(x+mu,2.)/(2*pow(sigma,2.)));
	//return a+b;
	return 0.;
}

void translation(Random* rnd)
{
	totalTranslations++;
	...
	for(int i=0;i<last;i++)
	{
		...
		newcorr = potential_density_matrix(positions[i]+delta,positions[inext]+delta); //Propongo un nuovo passo del Metropolis.
	...
	double acceptance_probability = exp(-acc_density_matrix_difference);//=probabilità nuova/probabilità vecchia.
...
void brownianBridge(Random *rnd)
{
	totalBB++;
...
with the gaussian sampling of the kinetic part of the density matrix. */
void brownianMotion(int which, Random* rnd) // BM is called only for PIGS simulations
{
