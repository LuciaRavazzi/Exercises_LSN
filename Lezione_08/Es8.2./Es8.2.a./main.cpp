...
int main(){
  ...
  double passo=0.001;
  double sigmamax=0.7;
  double sigmamin=sigma;
  ofstream fileout;
  fileout.open("dati.dat");
  

 cout << "Media" << " "<< "sigma" << " "<< "media_prog[i]" << " "<< "err_prog[i]" << endl;
do{
 sigma=0.6;
 do{
  Input();
  ...
  Statistica();
  if(mu == 0.803 && sigma == 0.612) Print();
  cout << mu << " "<< sigma << " "<< media_prog[N-1] << " "<< err_prog[N-1] << endl;
  fileout << mu << " "<< sigma << " "<< media_prog[N-1] << " "<< err_prog[N-1] << endl;
  sigma += passo;
...
void Statistica(void){
  for(int i=0; i<N; i++){
    ...
    //cout << "i: " << i << "  media_prog[i] " <<  media_prog[i] << " err_prog[i] " << err_prog[i] << endl;
  }
}
...
void Print(){
 ...
 for(int i=0; i<N; i++) fileout << i << " " << media_prog[i] << " " << err_prog[i] << endl;
