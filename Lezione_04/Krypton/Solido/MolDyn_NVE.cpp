...
double const kb=1.380658e-23;

int main(){ 
  double conv=164;
  double sigma=0.364*pow(10,-9);  
 ...
 Statistica(ekin,a,conv*kb);
 string b="output_epot.dat"; 
 Statistica(epot,b,conv*kb);
 string c="output_etot.dat"; 
 Statistica(etot,c,conv*kb);
 string d="output_temp.dat"; 
 Statistica(T,d,conv); 
 string e="output_press.dat";
 Statistica(press,e,conv*kb*(1./pow(sigma,3)));
...
//Kinetic energy
  for (int i=0; i<npart; ++i){	
    ...
    stima_press= rho*stima_temp+(1./(3.*vol))*sum;
...
void Statistica(double* media,string file, double fatt){
 ofstream out;
 out.open(file.c_str());
 ...
 for(int i=0; i<N; i++)
  ...
  out << i << " " << sum_prog[i]*fatt << " " << err_prog[i]*fatt << endl;
 }
